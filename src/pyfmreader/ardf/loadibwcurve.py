...
def loadIBWcurve(header, idx=0):
    ...
    height_measured = data['height (measured)']  # m
    height_piezo = data['height (piezo)'] # m
    ...
    appsegment.velocity = header['speed approach']
    ...
    retsegment.velocity = header['speed retract']
    retsegment.sampling_rate = header['rate retract']
    retsegment.z_displacement = header['z range']
