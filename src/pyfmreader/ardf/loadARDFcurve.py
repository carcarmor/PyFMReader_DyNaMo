...
def loadARDFcurve(idx, header):
    ...
    appsegment.segment_formated_data = {
        'height': channel_data_deflection[pnt_list[0]:pnt_list[1]], 
        'vDeflection': channel_data_piezo[pnt_list[0]:pnt_list[1]],
    ...
    appsegment.velocity = header['Notes']['ApproachVelocity']
    appsegment.sampling_rate = header['Notes']['NumPtsPerSec']
    appsegment.z_displacement = header['Notes']['ExtendZ']

    # Assing data and metadata for Retract segment.
    retsegment.segment_formated_data = {
        'height': channel_data_deflection[pnt_list[1]+1:len(channel_data_deflection)],
        'vDeflection': channel_data_piezo[pnt_list[1]+1:len(channel_data_deflection)],
    ...
    retsegment.velocity = header['Notes']['RetractVelocity']
    retsegment.sampling_rate = header['Notes']['NumPtsPerSec']
    retsegment.z_displacement = header['Notes']['RetractZ']
